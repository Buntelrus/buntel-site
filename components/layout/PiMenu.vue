<template>
  <div class="modal" :class="{'is-active': data.isActive}">
    <div class="modal-content has-text-centered">
      <menu class="pi-menu has-text-centered" :style="{'--a': numberItems}">
        <router-link class="button" v-for="(item, i) in menu" :key="item.url"
                     ...
                     :class="{'is-hovered': data.hoveredItem === i}"
                     :style="{'--i': i + 1}">
          {{ item.text }}
        </router-link>
      </menu>
...
const numberItems = computed<number>(() => menu.length)

const onMouseMove = (event: MouseEvent) => data.hoveredItem = calculateItemIndex(event)
const onMouseDown = (event: MouseEvent) => {
  toggleModal()
  const url = menu[calculateItemIndex(event)].url
  router.push(url)
}
...
function calculateItemIndex(event: MouseEvent) {
  ...
  const dPerItem = 360 / numberItems.value
...
.pi-menu
  ...
  --rel: #{$spaceBetween}
  /* 1 = one item size */
  --r: calc(.5 * (1 + var(--rel)) * var(--imax) / (3.60 / var(--a)))
  /* circle radius */
  --s: calc(2 * var(--r) + var(--imax))
  /* container size */
  --p: calc(1turn / var(--a))
  /* turn per item */
